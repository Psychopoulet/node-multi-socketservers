	...
	removeAllListeners(eventName) {
		...
		if (!eventName) {
			...
			this.servers.servers.forEach((server) => {
				...
				if (server.sockets && server.sockets.sockets && server.sockets.sockets.sockets) {

					for (let key in server.sockets.sockets.sockets) {
						server.sockets.sockets.sockets[key].removeAllListeners(eventName);
					}
			...
			return new Promise((resolve) => {
				...
				this.servers.forEach((server) => {
					...
					if (server.sockets) {
						...
						if (server.sockets.sockets && server.sockets.sockets.sockets) {

							for (let key in server.sockets.sockets.sockets) {
								server.sockets.sockets.sockets[key].disconnect(true);
								delete server.sockets.sockets.sockets[key];
							}
